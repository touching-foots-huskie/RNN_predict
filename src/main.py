...
def main():
    ...
    # reverse version:
    config['reverse'] = 'left' # left right none
    config['core_nn'] = 1  # choose inside
    config['m'] = 3  # previous m x # in reverse version we reverse the m and n
    config['n'] = 2  # previous n y
    ...
    if not config['test_mode']:
        ...
        dataX = plant.m_signal(config['sample_num'], 'signal_data/train_{}'.format(config['ptype']))
        dataY = plant.m_signal(config['sample_num'], 'signal_data/out_train_{}'.format(config['ptype']))
    val_dataX =  plant.m_signal(config['batch_size'], 'signal_data/val_{}'.format(config['ptype']))
    val_dataY =  plant.m_signal(config['batch_size'], 'signal_data/out_val_{}'.format(config['ptype']))
